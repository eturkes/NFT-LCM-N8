---
title: "NFT LCM Single Cell Analysis"
knit: (function(inputFile, encoding) {
  rmarkdown::render(
    inputFile,
    encoding = encoding,
    output_file = file.path("..", "results", "NFT_LCM_single-cell.html")
  )})
---
...
packages <- c(
  "DEP", "SummarizedExperiment", "plotly", "limma", "dplyr", "DT", "ComplexHeatmap", "colorRamp2",
  "RColorBrewer", "tibble", "scales", "GSEABase", "Rcpp", "Matrix", "edgeR", "SingleCellExperiment",
  "ProteoMM"
)
...
data_dir <- file.path("..", "data", "single-cell")
cache_dir <- file.path("..", "cache", "single-cell")
...
# data <- data[ , c(1:23, 28:29)]
data <- data[ , c(1:29)]

replace <- c(
  "D3", "D5", "D7", "D9", "D11", "D13", "D15", "D17", "D19", "F3", "F5", "F7", "F9", "F11", "F13", "F15", "H3",
  "H5", "H7", "H9", "H11", "H13", "H15", "H17"
)
colnames(data) <- c(colnames(data)[1:5], replace)
colnames(data) <- c(
  colnames(data)[1:5], paste(replace, rep(c("Pos", "Neg"), times = length(replace) / 2), sep = "_")
)

data <- data[ , c(1:5, seq(7, 29, by = 2), seq(6, 28, by = 2))]
# ---------------------------------------------
...
experimental_design <- data.frame(
  label = colnames(data)[6:29],
  condition = sub("^[^_]*_([^_]*).*", replacement = "\\1", colnames(data)[6:29]),
  donor = sub("_.*", replacement = "", colnames(data)[6:29])
)
experimental_design$replicate <- experimental_design$donor
experimental_design$donor <- paste0("Donor", rep(seq(12), times = 2))
# -----------------------------------------------------------------------

# Create SummarizedExperiment object for use with DEP.
# ----------------------------------------------------
data <- make_se(data, columns = 6:29, expdesign = experimental_design)
# ----------------------------------------------------

# remove <- which(data$donor == "Donor10")
# data <- data[ , -remove]
# 
# remove <- which(data$donor == "Donor11")
# data <- data[ , -remove]
...
# data <- data_bak # Make the main data object the unprocessed version.
...
discard <- which(rowSums(missing_data, TRUE) < 6) # TODO: Automate this.
if (length(discard) > 0) {
...
Neg_MAR <- rownames(Neg_data)[
  rownames(Neg_data) %in% Pos_MNAR
]
Pos_MAR <- rownames(Pos_data)[
  rownames(Pos_data) %in% Neg_MNAR
]
...
# -----------------------------------------------------------------
orig <- data # Make a copy of the pre-normalised data for plotting later.
# assay(data, withDimnames = FALSE) <- normalizeCyclicLoess(assay(data))
# assay(data, withDimnames = FALSE) <- normalizeQuantiles(assay(data))
# data <- normalize_vsn(data)
m_prot.info <- data.frame(a = paste('ID_',seq_len(nrow(data)), sep=""), b = paste('ID_',seq_len(nrow(data)), sep=""))
hs_m_ints_eig1 = eig_norm1(m=assay(data),treatment=factor(data$condition),prot.info=m_prot.info)
hs_m_ints_eig1$h.c = 2
hs_m_ints_norm_1bt = eig_norm2(rv=hs_m_ints_eig1)
assay(data, withDimnames = FALSE) <- as.matrix(hs_m_ints_norm_1bt$normalized[ , 3:ncol(hs_m_ints_norm_1bt$normalized)])
...
data_ensembl <- assay(data)
gene_anno_sub <- gene_anno[gene_anno$symbol %in% rownames(data_ensembl), ]
data_ensembl <- data_ensembl[rownames(data_ensembl) %in% gene_anno_sub$symbol, ]
gene_anno_sub <- gene_anno_sub[match(rownames(data_ensembl), gene_anno_sub$symbol), ]
rownames(data_ensembl) <- gene_anno_sub$ensembl
data_ensembl_orig <- data_ensembl

# add <- unique(unlist(geneIds(gene_sets))[which(unlist(geneIds(gene_sets)) %notin% rownames(data_ensembl))])
# # min <- mean(c(min(data_ensembl[ , 1:10]), min(data_ensembl[ , 11:20])))
# null_mat <- matrix(0, length(add), ncol(data_ensembl))
# rownames(null_mat) <- add
# data_ensembl <- rbind(data_ensembl, null_mat)

keep <- unique(unlist(geneIds(gene_sets)))[
  unique(unlist(geneIds(gene_sets))) %in% rownames(data_ensembl)
]
# remove <- which(
#   sapply(geneIds(gene_sets), FUN = function(x) any(x %in% remove))
# )
keep <- sapply(geneIds(gene_sets), FUN = function(x) sum(x %in% keep) / length(x))
keep <- which(keep >= 0.5)
...
#   sapply(geneIds(gene_sets), FUN = function(x) any(x %in% remove))
# )
keep <- sapply(geneIds(gene_sets), FUN = function(x) sum(x %in% keep))
keep <- which(keep >= 2)
gene_sets <- gene_sets[keep]

gene_set_list <- geneIds(gene_sets)
gene_set_list <- lapply(gene_set_list, function(x) x[x %in% rownames(data_ensembl)])

dup <- sapply(
  gene_set_list, FUN = function(x) paste(sort(x), collapse = "-")
)
dup <- which(duplicated(dup))
gene_set_list <- gene_set_list[-dup]

gene_sets <- gene_sets[names(gene_sets) %in% names(gene_set_list)]

gene_sets

data_ensembl_log <- data_ensembl_orig
data_ensembl <- Matrix(2 ^ data_ensembl, sparse = TRUE)
scores <- calculateScores(data_ensembl, rownames(data_ensembl), geneIds(gene_sets))
rownames(scores) <- names(gene_sets)
colnames(scores) <- colnames(data_ensembl)
# scores <- pmax(scores, 0)
scores_log <- log2(scores + 10)
```
...
```{r}
gse <- SingleCellExperiment(list(counts = scores, logcounts = scores_log))
gse$tau <- factor(ifelse(data$condition == "Neg", yes = "CTRL", no = "NFT"))
gse$donor <- data$donor
gse$tau_donor <- factor(paste(gse$tau, gse$donor, sep = "_"))
...
deg <- SingleCellExperiment(list(counts = data_ensembl_orig, logcounts = data_ensembl_log))
deg$tau <- ifelse(data$condition == "Neg", yes = "CTRL", no = "NFT")
deg$donor <- data$donor
...
gene_anno_sub <- gene_anno_sub[match(rownames(results_deg), gene_anno_sub$ensembl), ]
