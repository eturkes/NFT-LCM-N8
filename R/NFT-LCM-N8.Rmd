...
```{r}
assays(data) <- list(counts = assay(data))
dge <- SE2DGEList(data)

meta <- data.frame(tau = data$condition, donor = data$donor)
rownames(meta) <- colnames(data)
form <- ~ 0 + tau + (1 | donor)
cont_mat <- makeContrastsDream(
  form, data = meta, Pos_Neg = "tauPos - tauNeg",
)

voom <- voomWithDreamWeights(
  dge, formula = form, data = meta,
  plot = TRUE, BPPARAM = MulticoreParam(availableCores())
)
fit <- dream(
  voom, formula = form, L = cont_mat,
  data = meta, BPPARAM = MulticoreParam(availableCores())
)
fit <- variancePartition::eBayes(fit, robust = TRUE)

results <- variancePartition::topTable(fit, coef = "Pos_Neg", number = Inf)
```
